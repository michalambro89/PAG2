...
# Funkcja heurystyki: oblicza odległość euklidesową z uwzględnieniem wagi krawędzi
def heuristic(node, target, nodes, adjacency_list, weight_factor = 2.0):
    x1, y1 = nodes[node]
    x2, y2 = nodes[target]
    euclidean_distance = ((x2 - x1) ** 2 + (y2 - y1) ** 2) ** 0.5

    # Dodajemy wagi krawędzi
    edge_weight = adjacency_list[node].get(target, 0) * weight_factor
    return euclidean_distance + edge_weight
...
adjacency_file = r"C:\Users\micha\Desktop\Studia GI\Studia GI 5\Programowanie Aplikacji Geoinformacyjnych 2\proj_1\proj_custom_path\projekt\listy_sasiedztwa_4.csv"
...
try:
    ...
    start_node = 4252  # Przykładowy start
    target_node = 29308  # Przykładowy cel
